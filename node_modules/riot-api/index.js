...
var RiotApi = module.exports = function (API_KEY) {
    ...
    this._cache = {};
    this.BASE_PATH = 'https://euw.api.riotgames.com/api/lol/';
};
...
RiotApi.prototype.getCachedJSONRequest = function (url, callback) {
    ...
    if (this._cache.hasOwnProperty(url)) {
        ...
        request({
            ...
            console.log("1. " + error + " 2. " + response.toString() + " 3. " + json);
...
RiotApi.prototype.getChampions = function (options, callback) {
    var region = (options.region || 'euw').toLowerCase();
    var url = this.BASE_PATH + region + '/v1.2/champion?freeToPlay=true&api_key=' + this.API_KEY;
    this.getCachedJSONRequest(url, function (json) {
...
RiotApi.prototype.getRecentGames = function (options, callback) {
    var region = (options.region || 'euw').toLowerCase();
    var url = this.BASE_PATH + 'match/v3/matchlists/by-account/' + options.summonerId + '/recent?api_key=' + this.API_KEY;
    if (options.hasOwnProperty('summonerId')) {
        this.getCachedJSONRequest(url, callback);
    } else if (options.hasOwnProperty('summonerName')) {
        var self = this;
        this.getSummoner(options, function (json) {
            if (json.hasOwnProperty('status')) callback(json);
            else {
                var summonerId = json[Object.keys(json)[0]].id;
                self.getCachedJSONRequest(self.BASE_PATH + region + '/v1.3/game/by-summoner/' + summonerId + '/recent?api_key=' + self.API_KEY, callback);
            }
        });
    } else {
        callback({});
